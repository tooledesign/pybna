...
SELECT
    ...
    ST_Buffer(blocks.{block_geom_col},{roads_tolerance}) AS geom
...
WHERE
    ...
    OR ST_Length(ST_Intersection(block_geom,road_geom)) > {min_road_length}
GROUP BY block_id
;
...
FROM tmp_blkrds2
WHERE blocks.{block_id_col} = tmp_blkrds2.block_id
;
