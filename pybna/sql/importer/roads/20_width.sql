...
CREATE TEMP TABLE tmp_width AS (
    SELECT
        osm.id,
        CASE
            WHEN osm.width LIKE '% ft' THEN substring(osm.width from '\d+\.?\d?\d?')::FLOAT
            WHEN osm.width LIKE '% m' THEN 3.28084 * substring(osm.width from '\d+\.?\d?\d?')::FLOAT
            ...
            WHEN substring(osm.width from '\d+\.?\d?\d?')::FLOAT < 50
                THEN 3.28084 * substring(osm.width from '\d+\.?\d?\d?')::FLOAT
            WHEN substring(osm.width from '\d+\.?\d?\d?')::FLOAT >= 50
                THEN substring(osm.width from '\d+\.?\d?\d?')::FLOAT
            END AS width
